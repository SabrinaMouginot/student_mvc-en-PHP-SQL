...
class Project
{
    ...
    {
        $sql = 'insert into project (name, description, client_name)';
        $sql = $sql . ' values (:name, :description, :client_name)';
        $stmt = $this->pdo->prepare($sql);
        $stmt->bindParam(':name', $this->name);
        $stmt->bindParam(':description', $this->description);
        $stmt->bindParam(':client_name', $this->client_name);
       /* $stmt->bindParam(':created_at', date_create());
        $stmt->bindParam(':updated_at', date_create());
        $stmt->bindParam(':school_year_id', 1);
        $stmt->bindParam(':email', date_create());
*/
        $stmt->execute();
